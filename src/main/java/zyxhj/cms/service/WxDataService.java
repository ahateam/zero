...
public class WxDataService extends Controller {
	...
	public WxDataService(String node) {
		...
		try {
			...
			wxConfig.setAppid(WxDataService.APPID);// APPid
			wxConfig.setSecret(WxDataService.APPSECRET);// AppSecret
...
//	
	...
	public WxMaConfig getWxMaInMemoryConfig() {
		return wxConfig;
	}
	...
	public APIResponse getAccessToken(//
		...
		// wxDataService.getWxMaService().getUserService().getUserInfo(sessionKey,
		// encryptedData, ivStr);
		// WxMpUser wxMpUser =
	...
	// public WxPayUnifiedOrderResult WxpayUnifiedOrder(String body, Integer money,
	// String spbillCreateIp,
	// String notifyUrl, String openId) throws Exception {
	// WxPayUnifiedOrderRequest request =
	// WxPayUnifiedOrderRequest.newBuilder().body(body).totalFee(money)
